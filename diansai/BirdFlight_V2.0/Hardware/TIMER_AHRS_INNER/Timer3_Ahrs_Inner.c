...
void Timer4_ImuInnerinit()
{
	...
	TIM_TimeBaseInitStructure.TIM_Period = 2499; 									//自动重装载值
	TIM_TimeBaseInitStructure.TIM_Prescaler = 83;  								//定时器分频
	TIM_TimeBaseInitStructure.TIM_CounterMode=TIM_CounterMode_Up; //向上计数模式
	TIM_TimeBaseInitStructure.TIM_ClockDivision=TIM_CKD_DIV1; 
...
float time_isee1,time_isee2,time_isee3,time_isee4;
//定时器三中断
void TIM3_IRQHandler(void)//60ms
{
	OS_ERR err;
	OSIntEnter();
...
void TIM4_IRQHandler(void)
{
	...
	if(TIM_GetITStatus(TIM4,TIM_IT_Update)==SET)
	{
		...
		OSSemPost(&AttitudeInner_proc,OS_OPT_POST_1,&err);
	}
	OSIntExit();
}
